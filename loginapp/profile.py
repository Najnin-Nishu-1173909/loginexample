...
def validate_new_password(password):
    """Validate a new password against LU-TODO requirements."""
...
def delete_profile_picture(filename):
    """Delete a profile picture from the server if it exists."""
...
@login_required
def profile():
    """Display the current user's profile-management page."""
...
@login_required
def edit_profile():
    """Update first name, last name and position."""
    ...
    # Update session values so navigation and greetings
    # immediately show the new name.
...
def update_profile_picture():
    """Add or replace the current user's profile picture."""
    ...
    if not allowed_profile_picture(uploaded_file.filename):
        flash(
            "Profile pictures must be PNG, JPG, JPEG, GIF or WEBP.",
            "danger",
        )
    ...
    new_filename = (
        f"user_{session['user_id']}_{random_part}."
        f"{file_extension}"
    )
    ...
    try:
        ...
        # Remove the newly uploaded file if the database
        # update fails.
    ...
    # Delete the old picture after the database update.
