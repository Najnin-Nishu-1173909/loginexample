...
def init_db(app: Flask, user: str, password: str, host: str, database: str,
    ...
    Args:
        ...
        user: Username used to connect to the database server.
        password: Password used to connect to the database server.
        host: Host name or IP address of the database server.
        database: Name of the database to connect to on the database server.
        port: Port used to connect to the database server (default `5432`).
...
def get_db():
    """Gets a database connection to use while serving the current Flask
    request.
    ...
    If you only need a cursor, and not a reference to the database, you can
    just call the `get_cursor()` function. There's no need to call `get_db()`
    first.
    ...
    if 'db' not in g:
        ...
        connection.autocommit = connection_params['autocommit']
...
def get_cursor():
    """Gets a new dictionary cursor to use while serving the current Flask
    request.
...
def close_db(exception = None):
    """Closes the database connection associated with the current Flask request
    (if any).
